...
function createRow(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        const itm = data[i];
        if (itm.cu_isAvailable == true) {//müşteri silinmediyse
...
function beginForm(key) {
    ...
    for (i = 0; i < itm.length; i++) {
        if (key == itm[i].cu_code) {
            if (itm[i].cu_isAvailable == false) {
                alert("Bu müşteri numarası eski bir müşteriye aittir." +
                    "Geri eklenebilmesi rahat olması amacıyla bilgiler müşteri ekle paneline girildi.")
            }
            fncCustomeUpdate(i)
            break;
        }
...
$("#araInput").keyup(function () {
    console.log("aratıldı.")
    const ara = $("#araInput").val()

    $.ajax({
        url: './customer-search-servlet?ara=' + ara,
        type: 'GET',
        dataType: 'Json',
        success: function (data) {
            createRow(data);
